...
dialogflowApp.intent('episodes_to_watch', async (conv, params, argument, status) => {
  ...
  if (!userId) {
    ...
    if (strs.length === 0) {
      ...
      const last = strs.pop();
      conv.ask(`You kan watch ${strs.join(', ')} and ${last}`);
    }
  }
});

dialogflowApp.intent('Next episode to air', async (conv, params, argument, status) => {
  ...
  const episode = await getNextAirEpisode(params.name as string);
  if (episode.type === 'no-show') {
    conv.ask('I sorry but I could not found the show ' + params.name);
  } else if (episode.type === 'no-episode') {
    conv.ask(`There is not upcoming episode for ${params.name} right now`);
  } else {
    const data = episode.data;
    conv.ask(`The next episode of ${params.name} will air ${data.aired} and will be called ${data.name}`);
  }
});
